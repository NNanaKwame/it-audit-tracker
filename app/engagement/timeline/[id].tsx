...
export default function EditMilestoneScreen() {
  ...
  const { state, updateMilestone } = useAudit();
  const router = useRouter();

  const milestone = state.milestones[id!];
  if (!milestone) return <Text style={{ padding: 20 }}>Milestone not found.</Text>;

  const [title, setTitle] = useState(milestone.title);
  const [phase, setPhase] = useState<MilestonePhase>(milestone.phase);
  const [dueDateTime, setDueDateTime] = useState<Date | null>(new Date(milestone.dueDateTime));
  const [notes, setNotes] = useState(milestone.notes);
  const [notifyEnabled, setNotifyEnabled] = useState(milestone.notifyEnabled);
